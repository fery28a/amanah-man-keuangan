...
// Mendefinisikan semua endpoint API yang dibutuhkan
const API_URLS = {
...
const Laporan = () => {
  ...
  // Fungsi untuk merender tabel laporan
  const renderTable = () => {
    ...
    if (!tableHeaders[selectedReport]) return null;

    if (data.length === 0) {
      return <p className="text-gray-500">Tidak ada data untuk laporan ini pada periode yang dipilih.</p>;
    }
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.map((item, index) => (
              <tr key={item._id}>
                ...
                {selectedReport === 'kasKeluar' && (
                   <>
                    <td className="px-6 py-4 text-sm text-gray-500">{new Date(item.tanggal).toLocaleDateString()}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">{item.deskripsi}</td>
                    <td className="px-6 py-4 text-sm font-semibold text-red-500">Rp{item.nominal.toLocaleString('id-ID')}</td>
                  </>
                )}
  ...
  // Fungsi untuk merender ringkasan total keuangan
  const renderSummary = () => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold text-gray-700">Total Hutang (Belum Lunas)</h2>
          <p className="text-3xl font-bold text-yellow-500 mt-2">Rp{totalHutang.toLocaleString('id-ID')}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold text-gray-700">Total Piutang (Belum Lunas)</h2>
          <p className="text-3xl font-bold text-blue-500 mt-2">Rp{totalPiutang.toLocaleString('id-ID')}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-md col-span-full">
          <h2 className="text-2xl font-semibold text-gray-700">Saldo Akhir Kas</h2>
    ...
    <div className="p-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setSelectedReport('summary')} className={`px-4 py-2 rounded-md font-semibold transition-colors duration-200 ${selectedReport === 'summary' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}>Ringkasan</button>
          <button onClick={() => setSelectedReport('kasMasuk')} className={`px-4 py-2 rounded-md font-semibold transition-colors duration-200 ${selectedReport === 'kasMasuk' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}>Kas Masuk</button>
      ...
      {selectedReport === 'summary' ? (
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold text-gray-700 capitalize">Laporan {selectedReport.replace(/([A-Z])/g, ' $1')}</h2>
            <button
