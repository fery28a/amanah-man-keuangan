...
const API_URL = '/api/piutang/lunas';

const PiutangLunas = () => {
  const { piutangLunasData, setPiutangLunasData } = useContext(DataKeuanganContext);
  const [filteredPiutang, setFilteredPiutang] = useState([]);
  ...
  useEffect(() => {
    const fetchPiutangLunasData = async () => {
      try {
        const res = await axios.get(API_URL);
        setPiutangLunasData(res.data.data);
      } catch (err) {
        console.error('Error fetching piutang lunas data:', err);
      }
    };
    fetchPiutangLunasData();
  }, [setPiutangLunasData]);

  useEffect(() => {
    if (piutangLunasData && Array.isArray(piutangLunasData)) {
      const filteredData = piutangLunasData.filter(item => {
      ...
      setFilteredPiutang(filteredData);
    }
  }, [piutangLunasData, selectedMonth, selectedYear]);

  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Piutang Lunas</h1>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-xl font-semibold text-gray-700 mb-4">Daftar Piutang Lunas</h2>
        {filteredPiutang.length === 0 ? (
          <p className="text-gray-500">Tidak ada data piutang lunas untuk bulan ini.</p>
        ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200 table-auto">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tanggal Lunas</th>
