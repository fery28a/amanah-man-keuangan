...
const API_URL = 'http://localhost:5019/api/kaskeluar';

const KasKeluar = () => {
  const { kasKeluarData, setKasKeluarData } = useContext(DataKeuanganContext);
  const [deskripsi, setDeskripsi] = useState('');
  const [tanggal, setTanggal] = useState('');
  const [nominal, setNominal] = useState('');
  const [editingKasKeluarId, setEditingKasKeluarId] = useState(null);
  ...
  const fetchKasKeluarData = async () => {
    try {
      const res = await axios.get(API_URL);
      setKasKeluarData(res.data.data);
    } catch (err) {
      console.error('Error fetching kas keluar data:', err);
    }
  };
  ...
  const handleAddKasKeluar = async (e) => {
    e.preventDefault();
    const newKasKeluar = {
      deskripsi,
      tanggal,
    ...
    try {
      if (editingKasKeluarId) {
        await axios.put(`${API_URL}/${editingKasKeluarId}`, newKasKeluar);
        setEditingKasKeluarId(null);
      } else {
        await axios.post(API_URL, newKasKeluar);
      }
      fetchKasKeluarData();
      setDeskripsi('');
      setTanggal('');
      ...
      console.error('Error saving kas keluar:', err);
    }
  };

  const handleEdit = (item) => {
    setEditingKasKeluarId(item._id);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      try {
        ...
        console.error('Error deleting kas keluar:', err);
      }
    }
  };
  ...
  const filteredKasKeluar = kasKeluarData.filter(item => {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Kas Keluar</h1>
      ...
      <form onSubmit={handleAddKasKeluar} className="bg-white p-6 rounded-lg shadow-md mb-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          >
            {editingKasKeluarId ? 'Simpan Perubahan' : 'Tambah Kas Keluar'}
          </button>
        </div>
      </form>

      <div className="bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-xl font-semibold text-gray-700 mb-4">Daftar Kas Keluar</h2>
        {filteredKasKeluar.length === 0 ? (
          <p className="text-gray-500">Tidak ada data kas keluar untuk bulan ini.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 table-auto">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredKasKeluar.map((item, index) => (
